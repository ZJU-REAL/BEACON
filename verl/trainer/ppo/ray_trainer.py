...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.MiGPO:
        from migpo.core_migpo import compute_migpo_advantage
        # Get MiGPO specific config from kwargs
        migpo_step_advantage_w = kwargs.get('migpo_step_advantage_w', 1.0)
        migpo_mode = kwargs.get('migpo_mode', 'mean_norm')
        ...
        advantages, returns = compute_migpo_advantage(
            ...
            step_rewards=data.batch['step_rewards'],
            response_mask=data.batch['response_mask'],
            actions=data.non_tensor_batch.get('actions', None),
            index=data.non_tensor_batch['uid'],
            traj_uid=data.non_tensor_batch['traj_uid'],
            trial_id=data.non_tensor_batch.get('trial_id', None),
            gamma=migpo_gamma,
            threshold=migpo_threshold,
            step_advantage_w=migpo_step_advantage_w,
            mode=migpo_mode,
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    elif self.config.algorithm.adv_estimator == AdvantageEstimator.MiGPO:
                        from migpo.core_migpo import compute_migpo_step_rewards
                        step_rewards_tensor = compute_migpo_step_rewards(
                            batch=batch,
                            gamma=self.config.algorithm.get('migpo', {}).get('gamma', 0.99),
                            threshold=self.config.algorithm.get('migpo', {}).get('threshold', 0.85),
                        )
                        batch.batch['step_rewards'] = step_rewards_tensor
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            migpo_step_advantage_w=self.config.algorithm.get('migpo', {}).get('step_advantage_w', 1.0),
                            migpo_mode=self.config.algorithm.get('migpo', {}).get('mode', 'mean_norm'),
                            migpo_gamma=self.config.algorithm.get('migpo', {}).get('gamma', 0.99),
                            migpo_threshold=self.config.algorithm.get('migpo', {}).get('threshold', 0.85),
                        )
